...
def table(data: List[Dict[str, Any]], keys: List[str]):
    ...
    for row in data:
        widths = [
            max(w, len(str(row.get(k, '')))) for w, k in zip(widths, keys)
        ]
                    ...
                    if output.nvprof:
                        if 'gpu_trace' in output.nvprof:
                            ...
                            for row in output.nvprof['gpu_trace']:
                                ...
                                d['SMem (B)'] = row['Static SMem']
                                d['DMem (B)'] = row['Dynamic SMem']
                                d['Size (MB)'] = row['Size']
                                d['Throughput (GB/s)'] = row['Throughput']
                                d['Name'] = row['Name']
...
def output_to_json(test: str, output: RunnerOutput,
    ...
    if isinstance(output, NvprofRunnerOutput):
        result['nvprof_raw'] = output.nvprof_raw
